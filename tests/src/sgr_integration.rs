...
fn test_control_character_integration() {
    ...
    assert_eq!(
        a_cell.glyph_id, b'A' as u32,
        "Expected 'A' at (0,0), got: {}",
        a_cell.glyph_id
    );

    let d_cell = grid.get_cell(0, 1).unwrap();
    assert_eq!(
        d_cell.glyph_id, b'D' as u32,
        "Expected 'D' at (0,1), got: {}",
        d_cell.glyph_id
    );
}
...
fn test_terminal_session_simulation() {
    ...
    assert!(
        dollar_cell.attrs.contains(CellAttrs::BOLD),
        "First $ should be bold, got attrs: {:?}",
        dollar_cell.attrs
    );
}
