...
fn test_simple_color_parsing() {
    ...
    for action in actions {
        if let ParseAction::CsiDispatch(CsiAction::Sgr(_)) = action {
            ...
            println!("Parser state after SGR: fg_color: {:?}, bg_color: {:?}, attrs: {:?}", 
    ...
    println!("Resulting cell: glyph_id: {}, fg_color: {:?}, bg_color: {:?}, attrs: {:?}", 
        cell.glyph_id, cell.fg_color, cell.bg_color, cell.attrs);
    
    // This should be red
    assert_ne!(cell.fg_color, Color::DEFAULT_FG, "Cell should have non-default foreground color");
